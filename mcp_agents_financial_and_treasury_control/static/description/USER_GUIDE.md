...
1. Descarga el ZIP correspondiente a tu versión:
   - Odoo 16: `mcp_agents_financial_and_treasury_control_odoo16_v16.0.1.7.3.zip`
   - Odoo 17: `mcp_agents_financial_and_treasury_control_odoo17_v17.0.1.7.3.zip`
   - Odoo 18: `mcp_agents_financial_and_treasury_control_odoo18_v18.0.1.7.3.zip`
   - Odoo 19: `mcp_agents_financial_and_treasury_control_odoo19_v19.0.1.7.3.zip`
...
2. En tu repositorio GitHub conectado a Odoo.sh, crea la carpeta `mcp_agents_financial_and_treasury_control` y sube el contenido del ZIP
...
unzip mcp_agents_financial_and_treasury_control_odoo18_v18.0.1.7.0.zip -d /opt/odoo/custom-addons/

# Instalar (sustituye odoo18_db por tu base de datos)
sudo -u odoo python3 /opt/odoo/odoo-bin -c /etc/odoo.conf \
  -d odoo18_db -i mcp_agents_financial_and_treasury_control --stop-after-init
```
...
# Descargar módulo
ZIP_NAME="mcp_agents_financial_and_treasury_control_odoo${ODOO_VER}_v${ODOO_VER}.0.1.7.0.zip"
...
MOD_ID=$(curl -s -b /tmp/odoo_cookies.txt -X POST \
  ...
  -d "{\"jsonrpc\":\"2.0\",\"method\":\"call\",\"params\":{
    ...
    \"args\":[[[\"name\",\"=\",\"mcp_agents_financial_and_treasury_control\"]]],
...
$ZipName = "mcp_agents_financial_and_treasury_control_odoo${OdooVer}_v${OdooVer}.0.1.7.0.zip"
...
```ini
# Cambiar esto:
server_wide_modules = base,web,mcp_agents_financial_and_treasury_control
# Por esto:
...
2. In your GitHub repository connected to Odoo.sh, create the folder `mcp_agents_financial_and_treasury_control` and upload the ZIP contents
...
MOD_ID=$(curl -s -b /tmp/odoo_cookies.txt -X POST \
  "${ODOO_URL}/web/dataset/call_kw" \
  -H "Content-Type: application/json" \
  -d "{\"jsonrpc\":\"2.0\",\"method\":\"call\",\"params\":{\"model\":\"ir.module.module\",\"method\":\"search_read\",\"args\":[[[\"name\",\"=\",\"mcp_agents_financial_and_treasury_control\"]]],\"kwargs\":{\"fields\":[\"id\",\"state\"],\"limit\":1}}}" | \
  python3 -c "import sys,json; r=json.load(sys.stdin)['result']; print(r[0]['id'] if r else '')")
...
| Module cannot be uninstalled | Listed in `server_wide_modules` | Remove `mcp_agents_financial_and_treasury_control` from `/etc/odoo.conf` → `server_wide_modules`, then restart Odoo |
