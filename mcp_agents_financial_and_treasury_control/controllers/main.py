...
_P = "mcp_agents_financial_and_treasury_control."
            ...
            try:
                ...
                with _Reg(candidate).cursor() as _cr:
                    _cr.execute(
                        ...
                        "WHERE name='mcp_agents_financial_and_treasury_control' AND state='installed'"
                    )
                    if not _cr.fetchone():
                        continue
                    _cr.execute(
...
class IATCMCPController(http.Controller):
    ...
    def mcp_endpoint(self, **_kwargs):
        ...
        if request.httprequest.method == "GET":
            return _json_resp({
                "name": "mcp_agents_financial_and_treasury_control",
                "version": "1.0.0",
